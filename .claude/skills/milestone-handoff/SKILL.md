...
3. **Document.** Dispatch `scribe`: write `docs/milestones/M<n>-<slug>/handoff.md`, append decisions to `docs/ARCHITECTURE.md`, update `CHANGELOG.md`, and record durable facts to memory. Refresh the apex landing page too — the `STATUS` chip and the `MILESTONES` ladder in `src/app/page.tsx` must name the milestone just closed and the next one.
