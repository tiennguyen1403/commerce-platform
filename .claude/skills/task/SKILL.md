...
2. **Branch.** From an up-to-date `development`: `feat/<n>-<slug>` (or `fix/`, `chore/`, `docs/`).
...
8. **PR.** Conventional-commit the work, push the branch, and `gh pr create --base development` with a body that `Closes #<n>` and lists what changed + how it was verified. Ensure CI is green before merging into `development`.
